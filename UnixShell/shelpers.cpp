...
std::vector<Command> getCommands(const std::vector<std::string>& tokens){
  ...
  for(int i = 0; i < ret.size(); ++i){
	...
	for(int j = first + 1; j < last; ++j){ // loops starting after the command
        if (tokens[j].find("$") != std::string::npos) { // check for environmental variables
            ret[i].argv.push_back(getenv(tokens[j].substr(1).c_str()));
            break;
        }
        if (tokens[j] == "<") { // input takes the value after the <
            if ((ret[i].fdStdin = open(tokens[j + 1].c_str(), O_RDONLY)) < 0) { // error check for open
                perror("Couldn't open input file");
            }
            break;
        }
        if (tokens[j] == ">") { // output takes the value after the >
            if ((ret[i].fdStdout = open(tokens[j + 1].c_str(), O_APPEND | O_WRONLY | O_TRUNC | O_CREAT, S_IRWXU)) < 0) { // error check for open
	...
	if(i > 0){ // multiple commands (pipes)
	    int commandspipe[2];

        if (pipe(commandspipe) == -1) { // error check pipe
            perror("Pipe failed");
        }

        ret[i].fdStdin = commandspipe[0];
